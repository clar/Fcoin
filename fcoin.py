...
class Fcoin:
    ...
    def get_market_ticker(self, symbol):
        ...
        return self.public_request(
            "GET", "market/ticker/{symbol}".format(symbol=symbol)
        )

    def get_market_depth(self, level, symbol):
        """get market depth"""
        return self.public_request(
            "GET", "market/depth/{level}/{symbol}".format(level=level, symbol=symbol)
        )

    def get_trades(self, symbol):
        """get detail trade"""
        return self.public_request(
            "GET", "market/trades/{symbol}".format(symbol=symbol)
        )
    ...
    def buy(self, symbol, price, amount):
        ...
        return self.create_order(
            symbol=symbol, side="buy", type="limit", price=str(price), amount=amount
        )

    def sell(self, symbol, price, amount):
        """buy someting"""
        return self.create_order(
            symbol=symbol, side="sell", type="limit", price=str(price), amount=amount
        )
    ...
    def cancel_order(self, order_id):
        ...
        return self.signed_request(
            "POST", "orders/{order_id}/submit-cancel".format(order_id=order_id)
        )

    def order_result(self, order_id):
        """check order result"""
        return self.signed_request(
            "GET", "orders/{order_id}/match-results".format(order_id=order_id)
        )
    ...
    def get_candle(self, resolution, symbol, **payload):
        ...
        return self.public_request(
            "GET",
            "market/candles/{resolution}/{symbol}".format(
